...
class TokenDisplayPage:
    ...
    def format_time_diff(self, expires_at):
        """格式化时间差显示"""
        if not expires_at:
            return "无"
            ...
            with session_scope() as session:
                ...
                for i, user in enumerate(users, 1):
                    ...
                    if user.token_expires_at:
                        ...
                        time_diff_str = self.format_time_diff(user.token_expires_at)
                        print(f"    - 剩余时间: {time_diff_str}")
                        
                        # 检查是否即将过期
                        now = datetime.now(timezone.utc)
                        if user.token_expires_at.tzinfo is None:
                            user_expires = user.token_expires_at.replace(tzinfo=timezone.utc)
                        else:
                            user_expires = user.token_expires_at
                        
                        if user_expires < now:
                            print("    ❌ Token已过期!")
                        elif (user_expires - now).total_seconds() < 300:  # 5分钟内过期
                            print("    ⚠️  Token即将过期!")
                    else:
                        print("    - Token过期时间: 无")
    ...
    def display_token_comparison(self):
        """显示token对比信息"""
        print("\n" + "="*80)
        print("Token对比信息")
        print("="*80)
        ...
        try:
            with session_scope() as session:
                ...
                for i, user in enumerate(users, 1):
                    ...
                    # Token状态
                    has_access = bool(user.access_token)
                    has_refresh = bool(user.refresh_token)
                    is_expired = False
                    ...
                    print(f"  Access Token: {'✅ 有' if has_access else '❌ 无'}")
                    print(f"  Refresh Token: {'✅ 有' if has_refresh else '❌ 无'}")
                    print(f"  Token状态: {'❌ 已过期' if is_expired else '✅ 有效' if user.token_expires_at else '❓ 未知'}")
                    
                    if user.token_expires_at:
                        time_diff_str = self.format_time_diff(user.token_expires_at)
                        print(f"  过期时间: {time_diff_str}")
...
async def main():
    """主函数"""
    print("="*80)
    print("飞书文档下载系统 - Token显示和刷新测试页面 (修复版)")
    print("="*80)
